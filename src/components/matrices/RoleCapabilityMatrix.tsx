...
export function RoleCapabilityMatrix() {
  ...
  let visibleCols = capabilities.filter((c) =>
  ...
  if (onlyAssigned) {
    visibleRows = visibleRows.filter((r) => {
      ...
      return visibleCols.some((c) => {
        if (r.capabilityIds.includes(c.id)) return true;
        return contained.some((cr) => cr.capabilityIds.includes(c.id));
      });
    });
                ...
                {visibleRows.map((r, rowIdx) => {
                  ...
                  return (
                    <tr key={r.id} className={rowIdx % 2 === 0 ? 'bg-white' : 'bg-[#f0f0f0]/50'}>
                      ...
                      {visibleCols.map((c, colIdx) => {
                        const isDirect = r.capabilityIds.includes(c.id);
                        const origins = isDirect
                          ? []
                          : contained.filter((cr) => cr.capabilityIds.includes(c.id));
