...
export function GroupRoleMatrix() {
  ...
  let visibleRows = groups.filter((g) =>
    g.name.toLowerCase().includes(rowSearch.toLowerCase())
  );
  let visibleCols = roles.filter((r) =>
    r.name.toLowerCase().includes(colSearch.toLowerCase())
  );

  // onlyAssigned filter
  if (onlyAssigned) {
    visibleRows = visibleRows.filter((g) =>
      visibleCols.some((r) => {
        if (g.roleIds.includes(r.id)) return true;
        const directRoles = roles.filter((dr) => g.roleIds.includes(dr.id));
        return directRoles.some((d) => getContainedRoles(d, roles).some((cr) => cr.id === r.id));
      })
    );
    visibleCols = visibleCols.filter((r) =>
      visibleRows.some((g) => {
        if (g.roleIds.includes(r.id)) return true;
        const directRoles = roles.filter((dr) => g.roleIds.includes(dr.id));
        return directRoles.some((d) => getContainedRoles(d, roles).some((cr) => cr.id === r.id));
    ...
    <div className="space-y-3">
          ...
          <div className="inline-block overflow-hidden shadow-sm border border-[#e5e7eb]">
            <table role="grid" className="border-collapse text-sm bg-white">
              ...
              <tbody onMouseLeave={cross.clear}>
                {visibleRows.map((g, rowIdx) => {
                  const directRoles = roles.filter((r) => g.roleIds.includes(r.id));
                      ...
                      >
                        <div className="flex items-center gap-1.5">
                          ...
                          <span className={`text-[10px] font-semibold px-1 rounded shrink-0 ${
                            ...
                            {(g.scope ?? 'intern') === 'extern' ? 'E' : 'I'}
                          </span>
                        </div>
                        {g.description && <p className="text-xs text-[#767676]">{g.description}</p>}
                      </td>
                      {visibleCols.map((r, colIdx) => {
                        const isDirect = g.roleIds.includes(r.id);
                        const origins = isDirect
                          ? []
                          : directRoles.filter((d) =>
                              getContainedRoles(d, roles).some((cr) => cr.id === r.id)
                            );
                        ...
                        return (
                          ...
                          >
                            <button
                              onClick={() => !isInherited && !readOnly && toggleGroupRole(g.id, r.id)}
                              ...
                              title={
                                isDirect
                                  ? `${g.name} hat Rolle ${r.name} – klicken zum Entfernen`
                                  : isInherited
                                  ? `Geerbt über: ${origins.map((o) => o.name).join(', ')}`
                                  : `Rolle ${r.name} zu ${g.name} hinzufügen`
                              }
                            >
                              {isDirect && (
      ...
      <p className="text-xs text-[#767676]">
        <span className="inline-flex items-center gap-1 mr-4">
          <span className="inline-block w-3 h-3 rounded bg-[#38b5aa]/20" /> Direkt zugewiesen
        </span>
